...
$header += "<title>Список действующих Операционных офисов кредитных организаций по состоянию на $dtToday</title>"
$header += "</head>"
$header += "<body>"
...
#-------------------------------------------------------------------------------------
Start-Transcript ‘f:\AdminDir\SpisokKO\15spisOperOffice_Opsr.log’ -force
...
$FileName += [String]$(Get-Date).Day+"-15_Список_Операционных_офисов_ОПСиР" 
# $FileName
			...
			if (!$licenseRevoked) {
				...
				if ( ($spcurItem["Тип кредитной организации"] -eq "Операционные офисы")){
					...
					if (!($spcurItem["Регион"] -eq "КО за пределами Калужской области")){
						...
						$bankArr += $spcurItem["Банк"] + ";"+$spcurItem["Заполните для сортировки"] + ";"+$NaimPiK+";"+$DatReg+";"+$PAddress
