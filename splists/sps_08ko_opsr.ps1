...
$header += 				'<tr class="z1" >'
$header += 								'<td colspan="4" >'
$header += 								'Список действующих КО и филиалов действующих КО по '
...
$body += $tcell+"$numbPP</td>"
$body += $tcell+$($str.split(";"))[1]+"</td>"
$body += $tcell+$($str.split(";"))[2]+"</td>"
...
#-------------------------------------------------------------------------------------
Start-Transcript ‘f:\AdminDir\SpisokKO\08spisKO_Opsr.log’ -force
...
$FileName += [String]$(Get-Date).Day+"-08_Список_KO_ОПСиР" 
# $FileName
		...
		for ($jj=0; $jj -lt $iCnt; $jj++){
			...
			if (!$licenseRevoked) {
				...
				if (($spcurItem["Регион"] -eq "Калужские банки") -and ($spcurItem["Головной банк"] -eq "Да")){
					...
					$bankArr += $spcurItem["Заполните для сортировки"] + ";"+$NaimPiK+";"+$DatReg+";"+$UAddress
				...
				if ( ($spcurItem["Тип кредитной организации"] -eq "Банки") -and ($spcurItem["Регион"] -eq "Филиалы")){
					...
					$filArr +=	$spcurItem["Заполните для сортировки"] + ";"+$NaimPiK+";"+$DatReg+";"+$PAddress
...
for ($i=0;$i -lt $fsort.Count;$i++){
	...
	$MyReport += Get-HTMLReportBodyShort $numPP 	$fsort[$i]
	$numPP++
}

$icntAll = $icnt2+$icnt1
$Filename+="($icntAll).html"
