...
$header += 				'<tr class="z1" >'
$header += 								'<td colspan="8" >'
$header += 								'Список действующих Операционных касс вне кассового узла  '
$header += 								'кредитных организаций по '
...
#-------------------------------------------------------------------------------------
Start-Transcript ‘f:\AdminDir\SpisokKO\04spisKass.log’ -force
...
$FileName += [String]$(Get-Date).Day+"-04_Список_ОперКасс_вне_Узла" 
# $FileName
			...
			if (!$licenseRevoked) {
				if ( ($spcurItem["Тип кредитной организации"] -eq "Операционные кассы")){
					...
					if (!($spcurItem["Регион"] -eq "КО за пределами Калужской области")){
						...
						$bankArr +=  $spcurItem["Открыть_"] +  $spcurItem["Наименование"] + $spcurItem["Банк"] + $rsymb+$BnkLogo +$rsymb+ $BnkLogoAlt+$rsymb+  $NaimPiK +$rsymb+ $NaimGolov+$rsymb+  $VidLic +$rsymb+ $DatReg+$rsymb+  $RegNom+$rsymb+  $NomFil+$rsymb+  $RukTbl+$rsymb+  $NiD_Dov +$rsymb+ $Srok_Dov +$rsymb+ $Srok_Dov2 +$rsymb+$TelTbl+$rsymb+  $UAddress +$rsymb+ $PAddress+$rsymb+  $Okpo+$rsymb+  $Inn+$rsymb+  $Kpp+$rsymb+  $Ogrn+$rsymb+  $OgrnDate+$rsymb+  $KS4et+$rsymb+  $Bik+$rsymb+  $Okved
...
#	$mailmessage.Subject = "Список филиалов с истекающим сроком действия доверенности. Отчет за "+$(Get-Date).ToShortDateString()
