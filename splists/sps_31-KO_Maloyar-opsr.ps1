...
$header += "<title>Список действующих кредитных организаций и филиалов кредитных организаций Малоярославецкого региона по состоянию на $dtToday</title>"
$header += "</head>"
$header += "<body>"
...
$header += 				'<tr class="z1" >'
$header += 								'<td colspan="4" >'
$header += 								'Список действующих кредитных организаций и филиалов '
$header += 								'кредитных организаций Малоярославецкого региона по '
...
#-------------------------------------------------------------------------------------
Start-Transcript ‘f:\AdminDir\SpisokKO\31KOMaloyar_Opsr.log’ -force
...
$FileName += [String]$(Get-Date).Day+"-31_КО_Малояр_ОПСиР" 
# $FileName

$region_spr = GET-CONTENT f:\AdminDir\SpisokKO\spavochniki\maloyar_sprav.txt
$region_str  = ""
					...
					if (($spcurItem["Регион"] -eq "Калужские банки") -and ($spcurItem["Головной банк"] -eq "Да")){
						...
						if (!($spcurItem["Регион"] -eq "КО за пределами Калужской области")){
					        ...
					        if (!($spcurItem["Регион"] -eq "КО за пределами Калужской области")){
	        	        			...
	        	        			$NaimPiK = $NaimPiK.Replace(";"," ")
		        	        		$PAddress = $PAddress.Replace(";"," ")
			        	        	$filArr +=	$reginAddress+";"+$spcurItem["Банк"] + ";"+$spcurItem["Заполните для сортировки"] + ";"+$NaimPiK+";"+$DatReg+";"+$PAddress
...
if ($icnt2 -gt 0){
     if ($filArr.count -eq 1){
	$bsort= $filArr # из-за глюка powershell при сортировке возвращает не массив а строку, если в исходном массиве один элемент.
