...
$header += 								'филиалов действующих кредитных организаций в г. Обнинске по '
...
$body += $tcell+"$NaimPiK<br/><br/></td>" # <img src="+'"'+$BnkLogo+'" alt="'+$BnkLogoAlt+'" /></td>'
...
#-------------------------------------------------------------------------------------
Start-Transcript ‘f:\AdminDir\SpisokKO\10spisKO-obn.log’ -force
...
$FileName += [String]$(Get-Date).Day+"-10_Список_KO_ОБНИНСК" 
# $FileName
...
$MyReport = Get-HTMLHeader
		...
		for ($jj=0; $jj -lt $iCnt; $jj++){
			...
			if (!$licenseRevoked) {
                ...
                $isobn		= $false
                if ($PAddress.length -gt 0){
					$isobn 		= $PAddress.ToLower().Contains("обнинск")
				}
                if (!$isobn) {
                      	if ($UAddress.length -gt 0){
							$isobn 		= $UAddress.ToLower().Contains("обнинск")	
						}
				}
