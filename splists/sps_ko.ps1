...
$dtToday = $(Get-Date).Day.ToString("D2")+"."+$(Get-Date).Month.ToString("D2")+"."+$(Get-Date).Year
...
$header += 				'<thead>'
$header += 				'<tr class="z1" >'
$header += 								'<td colspan="21" >'
$header += 								'Список действующих кредитных организаций и '
$header += 								'филиалов действующих кредитных организаций по '
...
$header +=								'<td>БИК</td>'
$header +=								'<td>ОКВЭД</td>'
$header +=								'<td>Свидетельство<br/>о постановке на учет<br/>в налоговом органе</td>'
$header +=								'<td>Дата свидетельства<br/>постановки на учет<br/>в налоговом органе</td>'
...
#-------------------------------------------------------------------------------------
Function Get-HTMLReportBody ($BnkLogo, $BnkLogoAlt, $numbPP, $NaimPiK, $NaimGolov, $VidLic, $DatReg, $RegNom, $NomFil, $RukTbl, $NiD_Dov, $Srok_Dov, $Srok_Dov2, $TelTbl, $UAddress, $PAddress, $Okpo, $Inn, $Kpp, $Ogrn, $OgrnDate, $KS4et, $Bik, $Okved, $innSvid , $innSvidDate){
...
$body += $tcell+"$Bik</td>"
$body += $tcell+"$Okved</td>"
$body += $tcell+"$innSvid</td>"
$body += $tcell+"$innSvidDate</td>"	
...
#-------------------------------------------------------------------------------------
Start-Transcript ‘c:\AdminDir\SpisokKO\01spisKO.log’ -force
cd "C:\Program Files\Common Files\Microsoft Shared\Web Server Extensions\15\CONFIG\POWERSHELL\Registration"

# Add-PsSnapin Microsoft.SharePoint.PowerShell
#[System.Reflection.Assembly]::LoadWithPartialName("Mocrosoft.SharePoint")
cd c:\AdminDir\SpisokKO

$FileName = "c:\AdminDir\SpisokKO\"+$(Get-Date).Year+"."
...
$FileName += [String]$(Get-Date).Day+"-01_Список_KO.html" 
# $FileName
$nsite="http://s29sps.region.cbr.ru"
$nlist= $nsite + "/deprts/nadz/Lists/List/view1.aspx"
$AlertEmailStr	= "dl.nadz.alert@kaluga.cbr.ru"
$ListLinkStr	= $nsite + "/deprts/nadz/Lists/List/DispForm.aspx?ID="
...
$headBordr = '<tr class="z3"><td colspan="21" >К Р Е Д И Т Н Ы Е &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; О Р Г А Н И З А Ц И И</td></tr>'
$MyReport += $headBordr


		 
		#$env:SPpath = "${env:CommonProgramFiles}\Microsoft Shared\web server extensions\15\"
                $path00 = "C:\Program Files\Common Files\microsoft shared\Web Server Extensions\15\ISAPI\Microsoft.SharePoint.dll"
		[System.Reflection.Assembly]::LoadFrom($path00)
		...
		$SpSite = $SpSite = New-Object -TypeName "Microsoft.SharePoint.SPSite" -ArgumentList $nsite;
		...
		for ($jj=0; $jj -lt $iCnt; $jj++){
			...
			$innSvid	= $spcurItem["Св-тво о пост. на учет в налоговые органы"]
			$innSvidDate =[string]($spcurItem["Дата св-ства пост. на учет в налог. орг."])
			if ($innSvidDate.length -gt 0){
				$innSvidDate = $([DateTime]($innSvidDate)).ToShortDateString()
				}
			...
			if (!$licenseRevoked) {
				if (($spcurItem["Регион"] -eq "Калужские банки") -and ($spcurItem["Головной банк"] -eq "Да")){
				    ...
				    $bankArr +=  $spcurItem["Заполните для сортировки"] + $rsymb+$BnkLogo +$rsymb+ $BnkLogoAlt+$rsymb+  $NaimPiK +$rsymb+ $NaimGolov+$rsymb+  $VidLic +$rsymb+ $DatReg+$rsymb+  $RegNom+$rsymb+  $NomFil+$rsymb+  $RukTbl+$rsymb+  $NiD_Dov +$rsymb+ $Srok_Dov +$rsymb+ $Srok_Dov2 +$rsymb+$TelTbl+$rsymb+  $UAddress +$rsymb+ $PAddress+$rsymb+  $Okpo+$rsymb+  $Inn+$rsymb+  $Kpp+$rsymb+  $Ogrn+$rsymb+  $OgrnDate+$rsymb+  $KS4et+$rsymb+  $Bik+$rsymb+  $Okved + $rsymb + $innSvid + $rsymb + $innSvidDate
		...
		for ($jj=0; $jj -lt $iCnt; $jj++){
			...
			if ($Srok_Dov.length -gt 0){
			    #$Srok_Dov = $([DateTime]($Srok_Dov)).ToShortDateString()
                            $Srok_Dov = $([DateTime]($Srok_Dov)).Day.ToString("D2")+"."+$([DateTime]($Srok_Dov)).Month.ToString("D2")+"."+$([DateTime]($Srok_Dov)).Year
			}
			$Srok_Dov2  = [string]($spcurItem["Срок действия доверенности 2"])
			if ($Srok_Dov2.length -gt 0){
			    #$Srok_Dov2 = $([DateTime]($Srok_Dov2)).ToShortDateString()
			    $Srok_Dov2 = $([DateTime]($Srok_Dov2)).Day.ToString("D2")+"."+$([DateTime]($Srok_Dov2)).Month.ToString("D2")+"."+$([DateTime]($Srok_Dov2)).Year
			}
			if ($spcurItem["Телефон"].length -gt 0){
				$TelTbl		= Get-HTMLPhoneCells $spcurItem["Телефон"]
				}
			...
			if (!$licenseRevoked) {
				...
				if ( ($spcurItem["Тип кредитной организации"] -eq "Банки") -and ($spcurItem["Регион"] -eq "Филиалы")){
					...
					$filArr += $spcurItem["Заполните для сортировки"] +$rsymb+ $BnkLogo +$rsymb+ $BnkLogoAlt+$rsymb+  $NaimPiK +$rsymb+ $NaimGolov+$rsymb+  $VidLic +$rsymb+ $DatReg+$rsymb+  $RegNom+$rsymb+  $NomFil+$rsymb+  $RukTbl+$rsymb+  $NiD_Dov +$rsymb+ $Srok_Dov +$rsymb+ $Srok_Dov2 +$rsymb+$TelTbl+$rsymb+  $UAddress +$rsymb+ $PAddress+$rsymb+  $Okpo+$rsymb+  $Inn+$rsymb+  $Kpp+$rsymb+  $Ogrn+$rsymb+  $OgrnDate+$rsymb+  $KS4et+$rsymb+  $Bik+$rsymb+  $Okved + $rsymb + $innSvid + $rsymb + $innSvidDate
					#$MyReport += Get-HTMLReportBody $BnkLogo $BnkLogoAlt $numPP  $NaimPiK  $NaimGolov  $VidLic  $DatReg  $RegNom  $NomFil  $RukTbl  $NiD_Dov  $Srok_Dov  $Srok_Dov2 $TelTbl  $UAddress  $PAddress  $Okpo  $Inn  $Kpp  $Ogrn  $OgrnDate  $KS4et  $Bik  $Okved
					$icnt2++
					#$numPP++
				}
...
for ($i=0;$i -lt $bsort.Count;$i++){
	...
	$Okved		= $strArr[23]
	$innSvid    = $strArr[24]
	$innSvidDate= $strArr[25]
	$MyReport += Get-HTMLReportBody $BnkLogo $BnkLogoAlt $numPP  $NaimPiK  $NaimGolov  $VidLic  $DatReg  $RegNom  $NomFil  $RukTbl  $NiD_Dov  $Srok_Dov  $Srok_Dov2 $TelTbl  $UAddress  $PAddress  $Okpo  $Inn  $Kpp  $Ogrn  $OgrnDate  $KS4et  $Bik  $Okved $innSvid $innSvidDate
...
$headBordr = '<tr class="z3"><td colspan="21" >Ф И Л И А Л Ы</td></tr>'
$MyReport += $headBordr
for ($i=0;$i -lt $fsort.Count;$i++){

	
	$strArr = $fsort[$i].split($rsymb)
	$BnkLogo 	= $strArr[1]
	$BnkLogoAlt	= $strArr[2]
	$NaimPiK	= $strArr[3]
	$NaimGolov	= $strArr[4]
...
# $BnkLogo +$rsymb+ $BnkLogoAlt+$rsymb+  $NaimPiK +$rsymb+ $NaimGolov+$rsymb+  $VidLic +$rsymb+ $DatReg+$rsymb+  $RegNom+$rsymb+  $NomFil+$rsymb+  $RukTbl+$rsymb+  $NiD_Dov +$rsymb+ $Srok_Dov +$rsymb+ $Srok_Dov2 +$rsymb+$TelTbl+$rsymb+  $UAddress +$rsymb+ $PAddress+$rsymb+  $Okpo+$rsymb+  $Inn+$rsymb+  $Kpp+$rsymb+  $Ogrn+$rsymb+  $OgrnDate+$rsymb+  $KS4et+$rsymb+  $Bik+$rsymb+  $Okved	

$MyReport+=Get-HTMLFooter
$MyReport | out-file -encoding UTF8 -filepath $Filename
$MyReport | out-file -encoding UTF8 -filepath "\\region.cbr.ru\29DFS\Inform\Список КО и ОК\Список КО.txt"
# $MyReport

if ($AlertBankName.Count -gt 0){
	$MailBody = Get-HTMLMailBody $AlertBankName $AlertBankLinks	$AlertDateDovOver
	...
	$mailmessage.To.add("ast@kaluga.cbr.ru")
	$mailmessage.To.add($AlertEmailStr)
	$mailmessage.Subject = "Список филиалов с истекающим сроком действия доверенности. Отчет за "+$(Get-Date).Day.ToString("D2")+"."+$(Get-Date).Month.ToString("D2")+"."+$(Get-Date).Year
