...
$header += "<title>Список действующих ППКО кредитных организаций Малоярославецкого региона по состоянию на $dtToday</title>"
$header += "</head>"
$header += "<body>"
...
$header += 				'<tr class="z1" >'
$header += 								'<td colspan="4" >'
$header += 								'Список действующих передвижных пунктов кассовых операций (ППКО) '
...
Function Filt-Address ($address,$a_addr, $a_region)
{
		...
		#write-host $a_addr
		#read-host
                if ($adr.trim().length -gt 0){

		for ($ii = 0; $ii -lt $a_addr.Count-1; $ii++)
		{
		   if ( $adr.Contains($a_addr[$ii])){
...
#-------------------------------------------------------------------------------------
Start-Transcript ‘f:\AdminDir\SpisokKO\36VSPMaloyar_Opsr.log’ -force
...
$FileName += [String]$(Get-Date).Day+"-36_ППКО_Малояр_ОПСиР" 
# $FileName
...
$MyReport = Get-HTMLHeader
		...
		for ($jj=0; $jj -lt $iCnt; $jj++){
			...
			$reginAddress = Filt-Address $spcurItem["Город"] $addr $region
