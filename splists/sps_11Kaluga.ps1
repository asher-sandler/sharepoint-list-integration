...
$header += "<title>Банковская система по г. Калуга по состоянию на $dtToday</title>"
$header += "</head>"
$header += "<body>"
...
#-------------------------------------------------------------------------------------
Start-Transcript ‘f:\AdminDir\SpisokKO\11spisKO-klg.log’ -force
...
$FileName += [String]$(Get-Date).Day+"-11_Список_KO_КАЛУГА" 
# $FileName
		...
		for ($jj=0; $jj -lt $iCnt; $jj++){
			...
			if (!$licenseRevoked) {
			    ...
			    if ($UAddress.length -gt 0){
					$isklg 		= $UAddress.ToLower().Contains("калуга")
				}
				if (!$isklg){

					if ($PAddress.length -gt 0){
						$isklg 		= $PAddress.ToLower().Contains("калуга")	
					}
				}
